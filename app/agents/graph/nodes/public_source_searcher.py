...
def public_source_searcher_node(state: Mapping[str, object]) -> dict[str, object | None]:
    ...
    """
    try:
        # Slightly wider fanout improves section coverage without making
        # retrieval latency explode.
        ...
        for q, purpose in query_inputs:
            try:
                results = search_public_web(q, max_results=8, search_depth="basic", timeout_s=10.0)
                for r in results:
